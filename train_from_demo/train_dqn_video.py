...
from dqn_agent_video import DQNAgent  # modified version

def train_dqn():
    print("="*60)
    print("DQN FINE-TUNE FROM VIDEO PRETRAIN")
    print("="*60)
    print("1. Open Geometry Dash (windowed, Level 1 selected).")
    print("2. Press Enter to start training...")
    input()

    env = GeometryDashEnv(target_size=(84,84), frame_skip=4, max_steps=2000)
    state_shape = (4, 84, 84)
    ...
    agent = DQNAgent(
        ...
        pretrained_path="bc_from_video.pth",  # <--- load BC weights
        lr=1e-4,          # lower LR for fine-tuning
        gamma=0.99,
        epsilon_start=0.5, # start with less exploration
        epsilon_end=0.01,
        epsilon_decay=0.995,
        buffer_size=30000,
        batch_size=64,
    ...
    episode = 0
    step_count = 0
    total_steps = 60000  # fewer steps because pretrained
    ...
    while step_count < total_steps:
        ...
        if episode_reward > best_reward:
            ...
            agent.save("dqn_finetuned_best.pth")
            print(f"   New best model saved! Reward: {episode_reward:.2f}")

        episode += 1

    agent.save("dqn_finetuned_final.pth")
