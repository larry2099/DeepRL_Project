...
class BCDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        state = torch.FloatTensor(state).unsqueeze(0)  # (1,120,120)
...
def main():
    ...
    model = SimpleCNN(input_channels=1, num_actions=3).to(device)
    optimizer = optim.Adam(model.parameters(), lr=1e-3)
    criterion = nn.CrossEntropyLoss()

    print("Training BC on video (3 actions: 0=none, 1=jump, 2=hold)...")
    ...
    for epoch in range(40):
        ...
        if (epoch+1) % 10 == 0:
            print(f"Epoch {epoch+1}/40, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}")
